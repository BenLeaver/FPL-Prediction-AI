...
def get_top_n(df: pd.DataFrame, element_type: int, n: int) -> pd.DataFrame:
    ...
    top_n = filtered.sort_values(
        by="total_points_predictions", ascending=False
    ).head(n)
...
def load_final_predictions(gw: int, year: str) -> pd.DataFrame:
    ...
    """
    file_path = os.path.join("outputs", "predictions", f"{gw}_{year}_v1b_predictions.csv")
...
if __name__ == "__main__":
    ...
    print("<==== Top 10 Predictions after GW1 ====>")
    show_top_players_by_position(final_df, 10)
